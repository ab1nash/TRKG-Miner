...
class Rule_Learner(object):
    def __init__(self, edges, neighbors, id2relation, inv_relation_id, dataset):
        """
    ...
    def create_acyclic_rule(self, walk):
        """
        Create a rule given an acyclic temporal random walk.
        The rule contains information about head relation, body relations,
        variable constraints, confidence, rule support, and body support.
        A rule is a dictionary with the content
        {"head_rel": int, "body_rels": list, "var_constraints": list,
        ...
        rule = dict()
        rule["rule_type"] = "acyclic"
        rule["head_rel"] = int(walk["relations"][1])
        rule["body_rels"] = walk["relations"]
        ...
        # print("rule ",rule)
        # currently this rule support is broken. but a conf=1
        # is good for testing acyclic rules.
        if rule not in self.found_rules:
            self.found_rules.append(rule.copy())
            (
                rule["conf"],
    ...
    def estimate_acyclic_confidence(self, rule, num_samples=500):
        """
        Estimate the confidence of the rule by sampling bodies and checking the rule support.
    ...
    def sample_anti_body(self, body_rels, var_constraints):
        ...
        return sample_successful, body_ents_tss
    def sample_acyclic_body(self, body_rels, var_constraints):
        """
        Sample a walk according to the rule body.
        body_rels for use: {pre_edge, *, post_edge}
        The sequence of timesteps should be non-decreasing.
        ...
        if len(filtered_edges):
            next_edge = filtered_edges[np.random.choice(len(filtered_edges))]
            body_ents_tss.append(next_edge[3])
            body_ents_tss.append(next_edge[2])
        else:
            sample_successful = False
        ...
        if len(filtered_edges):
            ...
            body_ents_tss.insert(0,next_edge[3])
            body_ents_tss.insert(0,next_edge[2])
        else:
            sample_successful = False
    ...
    def calculate_acyclic_rule_support(self, unique_bodies, head_rel):
        """
        Calculate the rule support. Check for each body if there is a timestamp
        (larger than the timestamps in the rule body) for which the rule head holds.
...
def verbalize_rule(rule, id2relation):
    ...
    rule_str = "{0:8.6f} c {1:4}  {2:4}  {3}(X0,X{4},T{5}) <- "
    obj_idx = [
        idx
    ...
    if rule["rule_type"] == "acyclic":
        rule_str = "{0:8.6f} a {1:4}  {2:4}  {3}(X1,X2,T1) <- "
